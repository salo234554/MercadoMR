...
import ProductListInteractiveOptionOne from "~/components/elements/products/ProductListInteractiveOptionOne";
...
export default function useProductGroupSearch() {
    const [mostrarZoom, setMostrarZoom] = useState("ps-layout__item");
    const [viewPrdGenericos, setViewPrdGenericos] = useState(false);
    ...
    useEffect(() => {
        let viewprdgenericos = JSON.parse(localStorage.getItem("viewprdgenericos"));
        setViewPrdGenericos(viewprdgenericos);
                ...
                if (source && source.length > 0) {
                    ...
                    source &&
                        source.map((item, index) => {
                            validar = unico.includes(item.id);
                            if (!validar) {
                                unico.push(item.id);
                                resprod.push(item);
                            }
                    ...
                    const items = resprod.map((item, index) => (
                        <div className={mostrarZoom} key={item.id}>
                            <ProductListInteractiveOptionOne product={item} iditem={index} />
                            {/*
                                index % 2 == 0 ?
