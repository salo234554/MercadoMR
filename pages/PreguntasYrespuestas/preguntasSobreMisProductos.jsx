...
export default function preguntasSobreMisProductos() {
    ...
    useEffect(() => {
        const obtenerPreguntas = async () => {
            ...
            try {
                ...
                const preguntasAgrupadas = res.data.listapreguntamisprd.reduce(
                    (acc, pregunta) => {
    ...
    const handleChange = (event, value) => {
        setPage(value);
        sessionStorage.setItem("pageselectPregSobreMisProd", value);
        setTimeout(() => {
            lologrr.current.scrollIntoView({
                behavior: "smooth",
                block: "start",
            });
        }, 100);
    ...
    useEffect(() => {
        ...
        const urlVerPregSobreMisProduct = sessionStorage.getItem(
            "urlVerPregSobreMisProduct"
        );
        const cameFromPregSobreMisProd = localStorage.getItem(
            "cameFromPregSobreMisProd"
        );

        const paginaGuardada = sessionStorage.getItem(
            "pageselectPregSobreMisProd"
        );

        if (
            (cameFromProductPage === "true" &&
                urlProduct?.includes("/product/")) ||
            (cameFromPregSobreMisProd === "true" &&
                urlVerPregSobreMisProduct?.includes(
                    "/PreguntasYrespuestas/Resp"
                ))
        ) {
            if (paginaGuardada) {
            ...
            localStorage.removeItem("cameFromPregSobreMisProd");
        } else {
            sessionStorage.removeItem("pageselectPregSobreMisProd");
            setPage(1);
        }
    }, []);
        ...
        <>
            <div ref={irA}>
                <Container title="Mi Cuenta">
                    ...
                    />
                    <div
                        className="ps-page ps-page--inner contMinHegihtLeftPregRealzSobreMis"
                        id="myaccount">
                            ...
                            <Grid container spacing={1}>
                                <Grid
                                    ...
                                    <Button
                                        ...
                                        <div className="menulatpublicacionNeeWSobreMis">
                        ...
                        <div className="container">
                            <div className={sombraOpen} ref={lologrr}>
                                <Grid
                                                ...
                                                (idpregunta, indexPregunta) => {
                                                    ...
                                                    const idProductoRuta =
                                                        preguntasGrupo[0]
                                                            .idProductoRuta;
                                                    const hayRespuesta =
                                                        preguntasRespuestasOrdenadas.some(
                                                            (
                                                                preguntaRespuesta
                                                            ) =>
