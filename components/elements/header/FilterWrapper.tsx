...
const VehicleWrapper = forwardRef<HTMLDivElement, Props>(
    ({ setContent, setVehicle, setMostrar, scrollRef }, ref) => {
        ...
        const itemselectgarage = useSelector(
            (state: any) => state.itemselectgarage.itemselectgarage
        );
        ...
        const onClose = async () => {
            ...
            setMostrar(false); // Establece en false para cerrar

            // Vuelve a establecer en true después de un breve intervalo
            setTimeout(() => {
                setMostrar(true);
            }, 10); // 100 ms de retraso (puedes ajustar este valor si es necesario)
        ...
        const searchVehiclesLocal = async () => {
            if (user) {
                ...
                if (parseInt(idvehgarage) === -2) {
                    setSelected(0);
                    localStorage.setItem("idvehgarage", JSON.stringify(0));
                } else {
                    if (formattedVehicles.length > 0) {
                        if (selected && selected === -1) {
                            //setSelected(formattedVehicles.sort((a, b) => a?.id - b?.id).at(-1)?.id);
                            setSelected(parseInt(idvehgarage));
                        } else if (selected < 0) {
        ...
        useEffect(() => {
            ...
            vehselect = localStorage.getItem("selectvehgarage");
            let selectvehgarage = null;

            if (!vehselect || vehselect == null || vehselect == "undefined" || vehselect == undefined) {
                selectvehgarage = null;
            } else {
                selectvehgarage = JSON.parse(localStorage.getItem("selectvehgarage"));
            }

            //if (!selected || selected === 0 || !vehicle || vehicle.length == 0)
            //console.log("VEHICLE : ", selectvehgarage);
            vehicle = selectvehgarage;

            setContent(
                (selected !== null || selected === 0) && vehicle
                    ? `${vehicle.brand}, ${vehicle.model}, ${vehicle.year}, ${vehicle.cilinder
                    }, ${vehicle.fuel}${vehicle.traction && vehicle.traction !== "indefinido"
                        ? ", " + vehicle.traction
                        : ""
                    }${vehicle.transmision &&
                    ...
                    : selectvehgarage
                        ? `${selectvehgarage.brand}, ${selectvehgarage.model}, ${selectvehgarage.year
                        }, ${selectvehgarage.cilinder}, ${selectvehgarage.fuel}${selectvehgarage.traction &&
        ...
        }, [selected, itemselectgarage]);

        /*
`${selectvehgarage.brand}, ${selectvehgarage.model}, ${selectvehgarage.year}, ${
                        selectvehgarage.cilinder
                    }, ${selectvehgarage.fuel}${
                        selectvehgarage.traction && selectvehgarage.traction !== "indefinido"
                            ? ", " + selectvehgarage.traction
                            : ""
                    }${
                        selectvehgarage.transmision &&
                                    ...
                                    <EditingContext.Provider
                                        ...
                                        {vehicles &&
                                            (view === state.Filter ? (
                                                ...
                                                <Select vehicles={vehicles} setMostrar={setMostrar} scrollRef={scrollRef} />
                                            ) : view === state.Delete ? (
