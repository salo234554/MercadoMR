...
const Filter = ({ vehicles, setIsLeaving, isLeaving }: Props) => {
    ...
    const [showModalMensajes, setShowModalMensajes] = useState(null);
    const [tituloMensajes, setTituloMensajes] = useState(null);
    const [textoMensajes, setTextoMensajes] = useState(null);
    ...
    const uploadVehicle = async () => {
        if (user) {
            dispatch(getCtlrVehSelected(0));
            localStorage.setItem("prevvehselect", JSON.stringify(0));
            localStorage.setItem("addvehgarage", JSON.stringify(false));
        }
        if (!isValid) {
        ...
        // if (parseInt(type) === 2 && (!brand || !body || !year || !model || !cilinder || !traction || !fuel || !transmision)) {alert(type)

        /*
        if (parseInt(type) === 2 && (!brand || !body || !model || !cilinder || !fuel || !transmision)) {
            setShowModalMensajes(true);
            setTituloMensajes("Agregar vehículo");
            setTextoMensajes("Heey, todos los campos son requeridos!");
            return;
        }
            */
        ...
        if (user) {
            const formattedVehicle: FormattedVehicle = {
                ...
                cilindrajemotor: newVehicle.ids.cilinder ? parseInt(newVehicle.ids.cilinder) : 0,
                ...
                tipocombustible: newVehicle.ids.fuel ? parseInt(newVehicle.ids.fuel) : 2,
    ...
    const cancellAddVeh = () => {
        setIsLeaving(true);
        let prevvehselect = JSON.parse(localStorage.getItem("prevvehselect"));
        localStorage.setItem("canceladdveh", JSON.stringify(true));
        localStorage.setItem("idvehgarage", JSON.stringify(prevvehselect));
        dispatch(getCtlrVehSelected(prevvehselect));
    }
