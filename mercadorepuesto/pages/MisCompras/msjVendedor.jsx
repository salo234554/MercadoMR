...
export default function msjVendedor({ UsuarioVendedor }) {
    ...
    if (typeof window !== "undefined") {
        if (router.query.producto) {
            ...
            console.log("PREGUNTACOMPRA  : ", producto);
    ...
    const sendMessage = async () => {
        ...
        if (producto && producto.UsuarioVendedor) {
            usuariovende = producto.UsuarioVendedor;
        } else {
            setTituloMensajes("Validación de mensaje");
            setTextoMensajes(
            ...
            await fetch(`${URL_BD_MR}83`, {
                ...
                if (response) {
                    if (response.status === 200) {
                        ...
                        //confirmarCrearCuenta(idmessage);
                        ...
                        setInputMessage("");
                        setImageName(0); // Restablecer el nombre de la imagen
    ...
    //console.log("DDASAS : ", emailVendedor)

    function confirmarCrearCuenta(idmessage) {
        //alert("entre")
        //console.log("DATXX : ", datosToken)
        //return
        let mensaje = "Pregunta relacionado con el producto : " + producto.nombreProducto + ", " + inputMessage;
        let enlace = URL_MK_MR + "loginaccount/?redirect=7&idventa=" + idmessage;
    ...
    const leerMensajes = async () => {
        ...
        try {
            ...
            //console.log("PREGUNTACOMPRA1111 : ", datax);

            let messag = [];
            datax &&
                datax.map((row, index) => {
                    if (
                        datauser.uid == row.usuariocompra &&
                        (row.estado == 21 ||
                            row.estado == 22 ||
                            row.estado == 24)
                    ) {
    ...
    */
    ...
    useEffect(() => {
        ...
        localStorage.setItem('cameFromMsjVendedor', 'true');
    }, []);
