...
function PreguntasSobreMisProductos(props) {
    ...
    const leerMensajes = async () => {
        ...
        try {
            ...
            const datax = response.data.listpregmisprdadmon;
            ...
            datax &&
                datax.map((row, index) => {
                    if (datosusuarios.tipousuario == 1) {
                       
                        const readUsersCompra = async () => {
                            params = {
                                usuario: row.uidcomprador,
                            };
    ...
    useEffect(() => {
        leerMensajes();
    }, [datareturn, datosusuarios]);

    const irDetalle = (row) => {
        console.error("ROW:", row);
        dispatch(getReturn(false));
        router.push({
            pathname: "./VerDetallePrgMisPrductos",
            query: {
            ...
            <ContainerAdmon title="Mi Cuenta">
                <div className="ps-page ps-page--inner" id="myaccount">
                    {messages.length > 0 ? (
                        <div>
                            <button
                                className="VerifOtraManerButon mt-10 ml-30"
                                onClick={() => mostrar()}>
                                Mostrar Datos
                            </button>
                        </div>
...
export default PreguntasSobreMisProductos;
